...
const changeCurrentPassword = asyncHandler(async (req, res) => {
    ...
    await user.save({ validateBeforeSave: false }); // to avoid running other validators like required fields etc.
...
const updateAccountDetails = asyncHandler(async (req, res) => {
    ...
    const updatedUser = await User.findByIdAndUpdate(
        ...
        {
            $set: {
                fullName,   // fullName: fullName also works
                email
            }
        },
        { new: true }
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { user }, "User cpver image updated successfully"));
});
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    const channel = await User.aggregate([
        {
            $match: { username: username?.toLowerCase() } // gives the user document of User model of that username to next stage of pipeline
        },
        {
            $lookup: {
                from: "subscribers", // collection name in db
                localField: "_id", // field from User model
                foreignField: "channel", // field from Subscriber model
                as: "subscribers" // new field to be added to user document
            }
        },
        {
            $lookup: {
                ...
                as: "subscribedTo" // new field to be added to user document
            }
        },
        {
            $addFields: {
    ...
    // console log channel to see the structure after aggregation
    if (!channel?.length) {
...
export {
    loginUser,
    logoutUser,
    refreshAccessToken, getCurrentUser, changeCurrentPassword, updateAccountDetails,
    updateUserAvatar,
